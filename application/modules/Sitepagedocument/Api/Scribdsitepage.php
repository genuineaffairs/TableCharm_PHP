 ...
 * @version    $Id: Scribdsitepage.php 2011-05-05 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
/* $Id: scribdsitepage.php 1 2010-02-02 09:36:11Z SocialEngineAddOns $ */
/* Unofficial Scribd PHP Class library : THIS FILE HAS BEEN DOWNLOADED FROM THE SCRIBD.COM */

class Scribdsitepage {
   ...
   * @return array containing doc_id, access_key, and secret_password if nessesary.
   */
  public function uploadFromUrl($url, $doc_type = null, $access = null, $rev_id = null, $download = "view-only", $secure = 1) {
    $getPackageDocument = Engine_Api::_()->sitepage()->getPackageAuthInfo('sitepagedocument');
    $url = empty($getPackageDocument) ? null : $url;
  ...
  public function getConversionStatus($doc_id) {
    global $sitepagedocument_getConversionStatus;
    $getPackageDocument = Engine_Api::_()->sitepage()->getPackageAuthInfo('sitepagedocument');
    if (empty($getPackageDocument)) {
      return;
    }
    $method = "docs.getConversionStatus";
    $params['doc_id'] = $doc_id;
    $result = $this->postRequest($method, $params);
    $isReturn = empty($sitepagedocument_getConversionStatus) ? null : $result['conversion_status'];
    return $isReturn;
  }
