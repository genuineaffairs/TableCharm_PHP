 ...
 if (!empty($moduletype)) { 
   if ($pageinvite_id) {
    ...
    $resource_type = explode('_', $resource_type);
     ...
     if (strlen(sprintf(Zend_Registry::get('Zend_Translate')->_('You are being invited to '.$visit.' my %s at: '), $page)) > 50) { 
		 $message = substr(sprintf(Zend_Registry::get('Zend_Translate')->_('You are being invited to '.$visit.' my %s at: '), $page), 0, 40) ;
		 $message .= '...'; 
		 $bodyTextTemplate = $message . ' ' . $inviteUrl;
	 }
	 else 
		$bodyTextTemplate = sprintf(Zend_Registry::get('Zend_Translate')->_('You are being invited to '.$visit.' my %s at: '), $page) . ' ' . $inviteUrl;
