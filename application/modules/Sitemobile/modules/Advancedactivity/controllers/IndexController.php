...
//    }
  ...
  public function likeAction() {
    ...
    if (empty($comment_id)) {
      $method = '';
      $this->view->feedLikeCount = $action->likes()->getLikeCount();
...
//$helper = 'advancedActivitySM';
//    // Redirect if not json context
//   $this->view->body = $this->view->$helper($action, array('viewAllComments' => true, 'noList' => $this->_getParam('nolist', false)), null, true);
  }
  ...
  public function commentAction() {
    ...
    try {
      ...
      $comment_id = $action->comments()->addComment($subject, $body)->comment_id;
...
// Set Params for Attachment
      ...
      if ($this->_getParam('post_to_facebooks', false)) {
        try {
          ...
          if ($facebookApi
                  && Seaocore_Api_Facebook_Facebookinvite::checkConnection(null, $facebookApi)) {
            $fb_data = array(
                'message' => $publishMessage,
            );
            if ($publishUrl) {
