...
<?php if (!empty ($this->communityad_id)) { ?>
	<script type="text/javascript">
		$(window).bind('load', function() {
			var url = sm4.core.baseUrl + 'core/widget/index/mod/communityad/name/sitemobile-ads';
			var request = $.ajax({    
				url : url,
				type: "GET", 
				dataType: "html",
          ...
          'show_ads': '<?php echo $this->show_ads?>',
          'columnHeight':	'<?php echo $this->columnHeight?>'
				},    
				success : function(response) {
					$('#'+ '<?php echo $this->communityad_id?>').html(response);	
...
<?php if (empty ($this->load_content) && $this->identity_temp) : ?>
	<div id="communityadid_widget_showads_<?php echo $this->identity_temp?>">
<?php endif;?>

<?php if (!empty ($this->load_content)) { ?>
<div class="ui-page-content"><div class="<?php if($this->carouselView): ?> iscroll_wapper <?php endif;?>">
	<ul class="p_list_grid <?php if($this->carouselView): ?> iscroll_container <?php endif;?>">
		<?php foreach ($this->communityads_array as $community_ad): ?>
			...
			?>
			<li style="height:<?php echo $this->columnHeight; ?>px;" class="iscroll_item">
				<a href="<?php echo $this->url(array('adId' => $encode_adId), 'communityad_adredirect', true); ?>"  <?php echo $set_target ?> class="ui-link-inherit">
						<div class="p_list_grid_top_sec">
							<div class="p_list_grid_img" style="background-color:grey;">
