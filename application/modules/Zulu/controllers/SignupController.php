 ...
 * @license    http://www.socialengine.com/license/
 */
class Zulu_SignupController extends Core_Controller_Action_Standard {
...
//        ;
        ...
        // Zulu hack to fall back to previous step
//    if($this->getRequest()->getParam('back') !== null) {
//        $prePlugin = null;
//        foreach($this->_helper->getHelper('formSequence')->getPlugins() as $plugin) {
//            if( $plugin->isActive() ) {
//                // If we meet active step, deactivate current step and activate previous step
//                $plugin->setActive(false);
//                if($prePlugin instanceof Core_Plugin_FormSequence_Abstract) {
//                    $prePlugin->setActive(true);
//                }
//                break;
//            }
//            $prePlugin = $plugin;
...
//    }
        ...
        if (!$this->_helper->formSequence()) {
            $step = 1;
            foreach ($formSequenceHelper->getPlugins() as $plugin) {
                if ($plugin->isActive()) {
                    break;
                }
                $step++;
            ...
            $this->view->steps['cur_step'] = $step;
            $this->view->steps['total_steps'] = count($formSequenceHelper->getPlugins());
            return;
        }
    ...
    public function verifyAction() {
        ...
        try {
            ...
            // Send an email to notify user if he didn't upload a profile photo
            if ($user->photo_id == 0) {
                $mailParams = array(
                    'object_link' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('controller' => 'edit', 'action' => 'profile'), 'zulu_extended', true),
                    'host' => $_SERVER['HTTP_HOST'],
                    'recipient_title' => $user->getTitle(),
                );
                Engine_Api::_()->getApi('mail', 'core')->sendSystem(
                        $user, 'zulu_no_profilephoto', $mailParams
                );
            }
