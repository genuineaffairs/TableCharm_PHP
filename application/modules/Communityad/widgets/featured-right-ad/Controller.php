 ...
 */
class Communityad_Widget_FeaturedRightAdController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.featured-right-ad');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->featured_rightad = $communityad_featured_rightad = Zend_Registry::get( 'communityad_featured_rightad' );
			if( empty($communityad_featured_rightad) ) {
				return $this->setNoRender();
			}
