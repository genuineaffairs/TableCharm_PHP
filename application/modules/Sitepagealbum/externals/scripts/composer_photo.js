...
Composer.Plugin.SitepagePhoto = new Class({
  ...
  doRequest : function() {
    this.elements.iframe = new IFrame({
      'name' : 'composeSitepagePhotoFrame',
      ...
      'events' : {
        'load' : function() {
          this.doProcessResponse(window._composeSitepagePhotoResponse);
          window._composeSitepagePhotoResponse = false;
        }.bind(this)
      }
    }).inject(this.elements.body);

    window._composeSItepagePhotoResponse = false;
    this.elements.form.set('target', 'composeSitepagePhotoFrame');
  ...
  doProcessResponse : function(responseJSON) {
    ...
    this.elements.preview = Asset.image(responseJSON.src, {
      'id' : 'compose-sitepagephoto-preview-image',
