 ...
 */
class Communityad_Widget_RightAdsController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.right-ads');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_right1_ads = $communityad_right1_ads = Zend_Registry::get( 'communityad_right1_ads' );
			if( empty($communityad_right1_ads) ) {
				return $this->setNoRender();
			}
