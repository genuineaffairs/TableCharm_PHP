 ...
 */
class Communityad_Widget_SponserdExtendedAdController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.sponserd-extended-ad');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_sponcered_extended = $communityad_sponcered_extended = Zend_Registry::get( 'communityad_sponcered_extended' );
			if( empty($communityad_sponcered_extended) ) {
				return $this->setNoRender();
			}
