 ...
 */
class Communityad_Widget_FeaturedExtendedAdController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.featured-extended-ad');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->featured_extend = $communityad_featured_extended = Zend_Registry::get( 'communityad_featured_extend' );
			if( empty($communityad_featured_extended) ) {
				return $this->setNoRender();
			}
