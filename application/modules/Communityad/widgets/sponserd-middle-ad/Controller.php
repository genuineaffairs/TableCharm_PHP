 ...
 */
class Communityad_Widget_SponserdMiddleAdController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.sponserd-middle-ad');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_sponcered_middle = $communityad_sponcered_middle = Zend_Registry::get( 'communityad_sponcered_middle' );
			if( empty($communityad_sponcered_middle) ) {
				return $this->setNoRender();
			}
