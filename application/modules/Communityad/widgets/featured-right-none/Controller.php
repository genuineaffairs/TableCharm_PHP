 ...
 */
class Communityad_Widget_FeaturedRightNoneController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
		$load_content = 0;
		$page_setting = Engine_Api::_()->communityad()->getNonWidgetLimit('communityad.featured-right-none');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->featured_rightadnone = $communityad_featured_rightadnone = Zend_Registry::get( 'communityad_featured_rightadnone' );
			if( empty($communityad_featured_rightadnone) ) {
