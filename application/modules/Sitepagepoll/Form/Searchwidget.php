 ...
 */
class Sitepagepoll_Form_Searchwidget extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setAction($view->url(array(), 'sitepagepoll_browse', true))->getDecorator('HtmlTag')->setOption('class', '');
  }
  ...
  public function getAdditionalOptionsElement() {
    ...
    $select_content = $coreContent_table->select()->where('name = ?', 'sitepagepoll.search-sitepagepoll');
    ...
    //$row = $this->_searchForm->getFieldsOptions('sitepage', 'search');
     ...
     if (in_array("4", $enabledColumns)) {
      $this->addElement('Text', 'search_poll', array(
              'label' => 'Poll Title',
              'order' => $row->order - 4,
      ));
     ...
     if (in_array("3", $enabledColumns)) {
      ...
      $show_multiOptions["creation_date"] = "Everyone's Polls";

			if(!empty($viewer_id)) {
				$show_multiOptions["my_poll"] = 'My Polls';
				$show_multiOptions["my_like"] = 'Polls of Pages I Like';
			}
			
      $sitepagePackageEnable = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.package.enable', 1);
      if($sitepagePackageEnable) {
				$show_multiOptions["sponsored poll"] = 'Sponsored Polls';
      }
      ...
      $this->addElement('Select', 'show', array(
              ...
              'onchange' => $this->gethasMobileMode() ? '' : 'searchSitepagepolls();',
              'order' => $row->order - 2,
      ));
