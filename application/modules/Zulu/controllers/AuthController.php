 ...
 * @license    http://www.socialengine.com/license/
 */
class Zulu_AuthController extends Core_Controller_Action_Standard
{
  public function loginAction()
  {
    // Already logged in
    if( Engine_Api::_()->user()->getViewer()->getIdentity() && $this->_getParam('from_app') != 1 ) {
    ...
    // Make form
    $this->view->form = $form = new Zulu_Form_Login();
...
//        ;
    ...
    // Check if user belongs to the current site
    ...
    // Set default site id
    if (!$site_id) {
      $site_id = SharedResources_Model_DbTable_Sites::DEFAULT_SITE_ID;
    }
    
    $user_select = $userTable->select()
      ->from($userTable)
      ->setIntegrityCheck(false)
      ->join($usersSitesTableName, "{$userTableName}.user_id = {$usersSitesTableName}.user_id")
      ->where('site_id = ?', $site_id);

    if (is_numeric($email)) {
      $user_select->where('phone_number = ?', $email);
    } else {
      $user_select->where('email = ?', $email);
    }

    $user = $userTable->fetchRow($user_select);
