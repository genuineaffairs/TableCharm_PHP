...
<?php if ($this->paginator->getTotalItemCount() > 0): ?>
    <ul class="ynvideo_frame ynvideo_videos_manage">
        <h3>
            <?php
        ...
        <?php foreach ($this->paginator as $item): ?>                
            <li>
                ...
                <div class='video_options'>
                    ...
                    <?php
                        if ($item->status == 1) {    
                            if (isset($this->item) && $this->item instanceof Advgroup_Model_Group) {
                                echo $this->htmlLink(array(
                                    'route' => 'video_general',
                                	'action' => 'add-to-group',
                                    'group_id' => $this->item->getIdentity(),    
                                    'video_id' => $item->getIdentity()                                
                                ), $this->translate('Add to Group'), array('class' => 'buttonlink icon_ynvideo_add_to_group'));
                            } 
                        }
                    ?>
                </div>
