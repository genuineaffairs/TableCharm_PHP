 ...
 * @license    http://www.socialengine.com/license/
 */
class Zulu_Widget_ProfileFieldsController extends Engine_Content_Widget_Abstract
{
...
//            && Engine_Api::_()->getDbTable('accessLevel', 'zulu')->isAllowed($subject, $viewer, 'view') !== Authorization_Api_Core::LEVEL_MODERATE )
    {
      return $this->setNoRender();
    }
    
    if (Engine_Api::_()->user()->isSiteAdmin($subject)) {
      return $this->setNoRender();
    }
    ...
    $this->view->fieldValueLoop = $fieldValueLoop = $this->view->zuluFieldValueLoop($subject, $fieldStructure);
    if (empty($fieldValueLoop)) {
      return $this->setNoRender();
    }
    return;
