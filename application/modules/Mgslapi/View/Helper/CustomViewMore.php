 ...
 * @license    http://www.socialengine.com/license/
 */
class Mgslapi_View_Helper_CustomViewMore extends Zend_View_Helper_HtmlElement
{
  ...
  public function CustomViewMore($string, $moreLength = null, $maxLength = null, 
      $lessLength = null, $nl2br = true)
  {
    ...
    $content = '<'
      . $this->_tag
      . ' class="view_more_text"'
      . '>'
      . $shortText
      . $this->view->translate('... &nbsp;')
      . '<a class="api_view_more_link" href="javascript:void(0);" onclick="$(this).parent().hide(); $(this).parent().next().show()">'.$this->view->translate('more').'</a>'
      . '</'
      . $this->_tag
      . '>'
      . '<'
    ...
    if( $strLen >= $lessLength ) {
      $content .= '<a class="api_view_less_link" href="javascript:void(0);" onclick="$(this).parent().hide(); $(this).parent().previous().show()">'.$this->view->translate('less').'</a>';
    }
