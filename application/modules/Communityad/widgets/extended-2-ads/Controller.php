 ...
 */
class Communityad_Widget_Extended2AdsController extends Engine_Content_Widget_Abstract
{
	...
	{ $load_content = 0;
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.extended-2-ads');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->extend_ad_2 = $communityad_extended_2 = Zend_Registry::get( 'communityad_extend2_ad' );
			if( empty($communityad_extended_2) ) {
				return $this->setNoRender();
			}
