...
class User_Plugin_Signup_Fields extends Core_Plugin_FormSequence_Abstract
{
  ...
  public function getFormMap($form)
  {
    $formMap = array();

    $fieldStructure = $form->getFieldStructure();

    foreach ( $fieldStructure as $fskey => $map ) {
      $field = $map->getChild();
      $value = $form->getProcessedValues()[$field->field_id];

      $formMap[$field->label] = $value;
    }
