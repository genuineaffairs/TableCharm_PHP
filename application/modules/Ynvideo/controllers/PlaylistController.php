...
class Ynvideo_PlaylistController extends Core_Controller_Action_Standard {
    ...
    public function init() {
        ...
        $this->_roles = array('owner', 'owner_member', 'owner_member_member', 'owner_network', 'registered', 'everyone');

        if (0 !== ($playlist_id = (int) $this->_getParam('playlist_id')) &&
                null !== ($playlist = Engine_Api::_()->getItem('ynvideo_playlist', $playlist_id)) &&
                $playlist instanceof Ynvideo_Model_Playlist) {
    ...
    public function quickCreateAction() {
        if (0 !== ($video_id = (int) $this->getRequest()->getParam('video_id')) &&
    ...
    public function addAction() {
        ...
        if (isset($video)) {
            ...
            try {
                ...
                $actionTable = Engine_Api::_()->getDbtable('actions', 'activity');
                $action = $actionTable->addActivity(
                        $viewer,
                        $playlist,
                        'ynvideo_playlist_add_video',
                        ''
                );
    ...
    public function viewAction() {
        ...
        $select->join($playlistAssocTblName, "$playlistAssocTblName.video_id = $videoTblName.video_id")
                ->where("$playlistAssocTblName.playlist_id = ?", $playlist->getIdentity())
