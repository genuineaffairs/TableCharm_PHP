  ...
  <?php if ($this->likes->getTotalItemCount() > 0): // COMMENTS -------  ?>
    ...
    <?php if ($this->page == 1): ?>
      <div class="sm-ui-popup-top ui-header ui-bar-a">
        <?php if (!empty($action)): ?> 
          <a data-iconpos="notext" data-icon="chevron-left" data-corners="true" data-shadow="true" class="ui-icon-left " onclick="$('#comment-activity-item-' + <?php echo $action->action_id ?>).css('display', 'block');
              $('#like-activity-item-' + <?php echo $action->action_id ?>).css('display', 'none');"><?php //echo $this->translate('back');?></a>
        <?php else : ?>
    ...
    <?php endif; ?>
        <a href="javascript:void(0);" data-iconpos="notext" data-icon="remove" data-corners="true" data-shadow="true" data-iconshadow="true" class="ui-icon-right close-feedsharepopup ps-close-popup" ></a>
