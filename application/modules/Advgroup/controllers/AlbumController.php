...
//		$max = Engine_Api::_() -> authorization() -> getAdapter('levels') -> getAllowed('group', $viewer, 'numberAlbum');
	...
	public function editAction() {
		...
		if (!$this -> getRequest() -> isPost()) {
			$form -> populate(array('album_id' => $album -> album_id, 'title' => $album -> title, 'description' => $album -> description, ));
			return;
		}
	...
	public function listAction() {
		...
		$form -> populate(array('search' => $params['search'], 'view' => $params['view'], 'order' => $params['order'], 'page' => $this -> _getParam('page', 1)));
...
//		}
	...
	public function deleteAction() {
		...
		return $this -> _forward('success', 'utility', 'core', array('messages' => array(Zend_Registry::get('Zend_Translate') -> _('Album deleted')), 'layout' => 'default-simple', 'parentRedirect' => Zend_Controller_Front::getInstance() -> getRouter() -> assemble(array('controller' => 'album', 'action' => 'list', 'subject' => $group -> getGuid(), 'album_id' => $album -> getIdentity()), 'group_extended', true), 'closeSmoothbox' => true, ));
	}
