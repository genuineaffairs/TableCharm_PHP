...
<script type="text/javascript">
  ...
  var tagProfileDocumentAction = function(tag){

		var form = document.getElementById('filter_form_profile_document');
		form.elements['tag'].value = tag;

    $('filter_form_profile_document').submit();
  }
...
<form id='filter_form_profile_document' method='get' action='<?php echo $this->url(array('action' => 'browse'), 'document_browse', true) ?>'>
...
<div class="seaocore_gutter_view">
	<div class='seaocore_gutter_view_title'>
		...
		<h3><?php echo $this->htmlLink($this->document->getHref(), $this->document->getTitle()) ?>
			<?php if($this->document->approved != 1): ?>
				<?php echo $this->htmlImage('application/modules/Seaocore/externals/images/disapproved.gif', '', array('class' => 'fright', 'title' => $this->translate('Not Approved'))) ?>
			<?php endif;?>
		</h3>
	</div>
  ...
  <div class="seaocore_gutter_view_stat">
    ...
    <?php if ($this->category_name != '' && $this->subcategory_name == '') : ?>
      ...
      <?php echo $this->htmlLink($this->url(array('category_id' => $this->document->category_id, 'categoryname' => Engine_Api::_()->getDbTable('categories', 'document')->getCategorySlug($this->category_name)), 'document_browse'), $this->translate($this->category_name)) ?>
    <?php elseif ($this->category_name != '' && $this->subcategory_name != ''): ?> 
      <?php echo $this->translate('Category:'); ?>
      <?php echo $this->htmlLink($this->url(array('category_id' => $this->document->category_id, 'categoryname' => Engine_Api::_()->getDbTable('categories', 'document')->getCategorySlug($this->category_name)), 'document_browse'), $this->translate($this->category_name)) ?>
      <?php if (!empty($this->category_name)): echo '&raquo;';endif; ?>      
      <?php echo $this->htmlLink($this->url(array('category_id' => $this->document->category_id, 'categoryname' => Engine_Api::_()->getDbTable('categories', 'document')->getCategorySlug($this->category_name), 'subcategory_id' => $this->document->subcategory_id, 'subcategoryname' => Engine_Api::_()->getDbTable('categories', 'document')->getCategorySlug($this->subcategory_name)), 'document_browse'), $this->translate($this->subcategory_name)) ?>
      <?php if(!empty($this->subsubcategory_name)): echo '&raquo;';?>
        <?php echo $this->htmlLink($this->url(array('category_id' => $this->document->category_id, 'categoryname' => Engine_Api::_()->getDbTable('categories', 'document')->getCategorySlug($this->category_name), 'subcategory_id' => $this->document->subcategory_id, 'subcategoryname' => Engine_Api::_()->getDbTable('categories', 'document')->getCategorySlug($this->subcategory_name),'subsubcategory_id' => $this->document->subsubcategory_id, 'subsubcategoryname' => Engine_Api::_()->getDbTable('categories', 'document')->getCategorySlug($this->subsubcategory_name)), 'document_browse'),$this->translate($this->subsubcategory_name)) ?>
	   	<?php endif; ?>
    <?php endif; ?>
   	...
   	<?php if (count($this->documentTags )):?>
    	...
    	<?php foreach ($this->documentTags as $tag): ?>
      	<a href='javascript:void(0);' onclick='javascript:tagProfileDocumentAction(<?php echo $tag->getTag()->tag_id; ?>);'>#<?php echo $tag->getTag()->text?></a>&nbsp;
    	<?php endforeach; ?>
  	<?php endif; ?>
  </div>
  <div class="seaocore_gutter_view_stat"> 
  ...
  <div class="seaocore_gutter_view_body">
	...
	<?php if ($this->document->status == 1): ?> 
		...
		<div class="documents_user_options">
			...
			<p>
				<?php if (!empty($this->viewer_id) && !empty($this->can_download) && $this->link && $this->document->download_allow && $this->view_download): ?>
					<?php echo $this->htmlLink($this->link, $this->translate('Download'), array (
						'class'=>'buttonlink icon_document_download', 'target' => '_blank')) ?>
				<?php endif; ?>

				<?php if(!empty($this->viewer_id) && !empty($this->can_email) && $this->document->email_allow == 1 && $this->document->status == 1 && $this->view_email): ?>
					<?php echo $this->htmlLink(array('route' => 'default', 'module' => 'document', 'controller' => 'index', 'action' => 'email', 'id' => $this->document->document_id), $this->translate('Email Document'), array(
						'class' => 'buttonlink smoothbox icon_documents_email',
					)) ?>
				<?php endif; ?>	

        <?php if(!empty($this->viewer_id) && $this->can_share == 1 && $this->document->status == 1 && $this->document->draft != 1 && $this->document->approved == 1): ?>
        	<?php echo $this->htmlLink(Array('module'=> 'activity', 'controller' => 'index', 'action' => 'share', 'route' => 'default', 'type' => 'document', 'id' => $this->document->getIdentity(), 'format' => 'smoothbox'), $this->translate("Share"), array('class' => 'smoothbox buttonlink seaocore_icon_share')); ?>
        <?php endif;?>

        <?php if($this->can_report == 1 && !empty($this->viewer_id)): ?>
					<?php echo $this->htmlLink(Array('module'=> 'core', 'controller' => 'report', 'action' => 'create', 'route' => 'default', 'subject' => $this->document->getGuid(), 'format' => 'smoothbox'), $this->translate("Report"), array('class' => 'smoothbox buttonlink seaocore_icon_report')); ?>
				<?php endif;?>

				<?php if($this->can_edit): ?>
					<?php if($this->document->draft == 1) echo $this->htmlLink(array('route' => 'document_publish', 'document_id' => $this->document->document_id), $this->translate('Publish Document'), array(
					'class'=>'buttonlink smoothbox icon_document_publish')) ?>
				<?php endif; ?>
			</p>
		</div>
		<?php if(!empty($this->can_social_share) && !empty($this->code)): ?>
			<div class="document_progile_socialshare">
				<?php echo $this->code;?>
			</div>
		<?php endif; ?>
  </div>

	<?php if(!empty($this->can_comment_like)):?>
		<?php Engine_Api::_()->core()->clearSubject('user'); ?>
		<?php Engine_Api::_()->core()->setSubject($this->document); ?>
		<?php echo $this->action("list", "comment", "core", array("type"=>"document", "id"=>$this->document->getIdentity())) ?>
		<?php Engine_Api::_()->core()->clearSubject('document'); ?>
		<?php Engine_Api::_()->core()->setSubject($this->user); ?>
	<?php endif; ?>