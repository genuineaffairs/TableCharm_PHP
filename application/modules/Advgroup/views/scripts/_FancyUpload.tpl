...
window.addEvent('domready', function() { // wait for the content
  ...
  var up = new FancyUpload2($('demo-status'), $('demo-list'), { // options object
    ...
    timeLimit: 0,
    limitFiles: <?php echo (!empty($this->element->limitFiles))?$this->element->limitFiles:'null' ?>,
    ...
    onFileRemove: function(file) {
        ...
        request = new Request.JSON({
        	'format' : 'json',
            'url' : '<?php echo $this->url(Array('module'=>'advgroup', 'controller' => 'photo', 'action'=>'remove'), 'default') ?>',
			'data': {
            	'photo_id' : file_id,
			},
...
<input type="hidden" name="<?php echo $this->name;?>"
	id="fancyuploadfileids" value="" />
<fieldset id="demo-fallback">
	<legend>
		<?php echo $this->translate('File Upload');?>
	</legend>
	<p>
		<?php echo $this->translate("CORE_VIEWS_SCRIPTS_FANCYUPLOAD_DESCRIPTION");?>
	</p>
	<label for="demo-photoupload"> <?php echo $this->translate('Upload a Photo:');?>
		<input type="file" name="Filedata" />
	</label>
	<?php if ($this->element->limitFiles) : ?>
	(
	<?php echo $this->translate('maximum')?>
	&nbsp;
	<?php echo $this->element->limitFiles?>
...
<div id="demo-status" class="hide">
	...
	<div>
		<a class="buttonlink icon_photos_new" href="javascript:void(0);"
			id="demo-browse"><?php echo $this->translate('Add Photos');?> </a> <a
			class="buttonlink icon_clearlist" href="javascript:void(0);"
			id="demo-clear" style='display: none;'><?php echo $this->translate('Clear List');?>
		</a>
	</div>
	<div class="demo-status-overall" id="demo-status-overall"
		...
		<img
			src="<?php echo $this->baseUrl() . '/externals/fancyupload/assets/progress-bar/bar.gif';?>"
...
<ul id="demo-list" style="margin-bottom:10px;"></ul>
