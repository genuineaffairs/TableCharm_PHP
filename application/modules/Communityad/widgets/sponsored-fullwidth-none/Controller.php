 ...
 */
class Communityad_Widget_SponsoredFullwidthNoneController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
		$load_content = 0;
		$page_setting = Engine_Api::_()->communityad()->getNonWidgetLimit('communityad.sponsored-fullwidth-none');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_sponcered_fullwidthnone = $communityad_sponcered_fullwidthnone = Zend_Registry::get( 'communityad_sponcered_fullwidthnone' );
			if( empty($communityad_sponcered_fullwidthnone) ) {
