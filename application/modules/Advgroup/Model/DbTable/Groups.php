  ...
  public function getGroupSelect($params = array())
  {
   ...
   if(isset($params['order']) && $params['order'] == 'displayname'){
       $select  -> setIntegrityCheck(false)
                -> join('engine4_users as u',"u.user_id = $groupName.user_id",'')
                -> order("u.displayname ".$params['direction']);
   }
       ...
       $select  ->setIntegrityCheck(false)
                ...
                ->order("COUNT('topic_id') ".$params['direction']);
    }
    else {
       // Order
    ...
    if (!empty($params['owner']))
     {
         ...
         $select -> setIntegrityCheck(false)
                 -> join('engine4_users as u1',"u1.user_id = $groupName.user_id",'')
    ...
    if(!empty($params['tag']))
    {
      $select
        ...
        ->joinLeft($tags_name, "$tags_name.resource_id = $groupName.group_id","")
        ->where($tags_name.'.resource_type = ?', 'group')
