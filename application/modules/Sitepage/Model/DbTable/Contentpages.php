 ...
 */
class Sitepage_Model_DbTable_Contentpages extends Engine_Db_Table implements Engine_Content_Storage_Interface {

  protected $_rowClass = "Sitepage_Model_Contentpage";

  public function loadMetaData(Engine_Content $contentAdapter, $name) {
  	
    $select = $this->select()->where('name = ?', $name)->orWhere('contentpage_id = ?', $name);
  ...
  public function loadContent(Engine_Content $contentAdapter, $name) {
    ...
    $select = $this->select()->where('page_id = ?', $sitepage_id)->where('name = ?', $name)->orWhere('contentpage_id = ?', $name);
    $page = $this->fetchRow($select);

    if (is_object($page)) {
			$contentTable = Engine_Api::_()->getDbtable('content', 'sitepage');
			$select = $contentTable->select()
							->where('contentpage_id = ?', $page->contentpage_id)
							->order('order ASC');
      ...
      $page_id = Engine_Api::_()->sitepage()->getWidgetizedPage()->page_id;
			$contentTable = Engine_Api::_()->getDbtable('admincontent', 'sitepage');
			$select = $contentTable->select()
							->where('page_id = ?', $page_id)
							->order('order ASC');
			$content = $contentTable->fetchAll($select);
			$structure = Engine_Api::_()->getDbtable('admincontent', 'sitepage')->prepareContentArea($content);
			$element = new Engine_Content_Element_Container(array(
  ...
  public function deletePage(Sitepage_Model_Page $page) {
    ...
    Engine_Api::_()->getDbtable('content', 'sitepage')->delete(array('contentpage_id = ?' => $page->contentpage_id));    
    $page->delete();
    return $this;
  }
  ...
  public function getContentPageId($page_id) {
  	...
  	$selectPageAdmin = $this->select()            
            ->from($this->info('name'), array('contentpage_id', 'description', 'keywords'))  
