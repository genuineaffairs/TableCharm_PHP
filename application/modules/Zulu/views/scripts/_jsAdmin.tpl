...
<script type="text/javascript">
  ...
  var registerEvents = function() {
    ...
    $$('body').removeEvents().addEvent('click', function(e){
        if(e.target && !$(e.target).getParent('.field_extraoptions')) {
            $$('.field_extraoptions').removeClass('active');
        }
    });
  }
  ...
  var genericUpdateKeysAndOrder = function(htmlArr, group_key) {
    consoleLog(htmlArr);
    $H(htmlArr).each(function(html, key) {
      var oldEl = $('admin_field_' + key);
      var newEl = Elements.from(html)[0];
      
      if(oldEl && newEl && (getPrevGroupElementId(oldEl) === false || getPrevGroupElementId(oldEl) === 'admin_field_' + group_key)) { // Only replace, no change in position
        consoleLog('replace', key);
        newEl.replaces(oldEl);
      } else {
        consoleLog('remove', key);
        ...
        if( newEl ) { // Add
          ...
          // This could cause future replaces
          var ids = readIdentifiers(key);
          var nextGroupId = getNextGroupElementId(group_key);

          if( ids.option_id == topLevelOptionId) {
            if(nextGroupId !== false) {
              var targetEl = $(nextGroupId);
              if( !targetEl ) {
                //throw '<?php echo $this->string()->escapeJavascript($this->translate("could not find target element")) ?>';
              } else {
                newEl.inject(targetEl, 'before');
              }
            } else {
              var targetEl = $$('.admin_fields')[0];
              if( !targetEl ) {
        ...
        // Only save order when having changes in element list.
        saveOrder();
      }
    });
    registerEvents();
  ...
  /*
   * Get next group starting element, also known as heading
   */
  var getNextGroupElementId = function(group_key) {
      var adminFieldGroup = $('admin_field_' + group_key);
      if(!adminFieldGroup) {
          return false;
      }
      var cur_item = adminFieldGroup.getNext('li');
      while(cur_item !== null) {
          if(cur_item.getChildren('.heading').length > 0 
                  // Exclude subheading from finding targets
                  && !cur_item.getChildren('.heading')[0].hasClass('subheading')) {
              return cur_item.get('id');
          }
          cur_item = cur_item.getNext('li');
      }
      return false;
  }
  
  var getPrevGroupElementId = function(el) {
      if(!el) {
          return false;
      }
      
      if(el.getChildren('.heading').length > 0) {
        return el.get('id');
      }
      
      var cur_item = el.getPrevious('li');
      while(cur_item !== null) {
          if(cur_item.getChildren('.heading').length > 0) {
              return cur_item.get('id');
          }
          cur_item = cur_item.getPrevious('li');
      }
      return false; 
  }
  ...
  var onFieldCreate = function(field, htmlArr, group) {
    //genericUpdateKeys(htmlArr);
    if(typeof group === 'undefined') {
      genericUpdateKeys(htmlArr);
    } else {
      genericUpdateKeysAndOrder(htmlArr, group);
    }
    
    jQuery.registerGridViewEvents();
  }
  ...
  var onFieldEdit = function(field, htmlArr, group) {
    //genericUpdateKeys(htmlArr);
    genericUpdateKeysAndOrder(htmlArr, group);
    jQuery.registerGridViewEvents();
  }
  ...
  var onHeadingEdit = function(field, htmlArr, group) {
//    genericUpdateKeys(htmlArr);
      genericUpdateKeysAndOrder(htmlArr, group);
  }
  ...
  (function($) {
    ...
    $(window).load(function() {
      var redLine = $('.admin_fields_options').offset().top + 20;
      $(this).scroll(function() {
        if($(this).scrollTop() > redLine) {
          if(!elementAppended) {
            var addQuesHtml = '<a href="javascript:void(0);" onclick="void(0);" class="admin_fields_options_addquestion">Add Question</a>';
            var addHeadHtml = '<a href="javascript:void(0);" onclick="void(0);" class="admin_fields_options_addheading">Add Heading</a>';
            $('body').append('<div id="floating_admin_fields_options">' + addQuesHtml + addHeadHtml + '</div>');
            registerEvents();
            elementAppended = true;
          }
          $('#floating_admin_fields_options').fadeIn('fast');
        } else {
          $('#floating_admin_fields_options').fadeOut('fast');
        }
      })
    })
  })(jQuery);
