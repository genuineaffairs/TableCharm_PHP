    ...
    {
        return 'email:'+ auth_email_address+', password:'+ auth_password+'feedId: '+feedId+ ' userId: '+userId+' baseurl'+baseurl;
...
//        });
    ...
    {
//        $.ajax(baseurl+'android/api/v3.0/postFeedUnlike', {email: auth_email_address, password: auth_password, feed_id: feedId, user_id: userId}).done(function(data) {
//            //alert("Data Loaded: " + data);
//        });
        $.ajax({
            type: "POST",
...
//        $.ajax(baseurl+'android/api/v3.0/'+action, {email: auth_email_address, password: auth_password, circle_id: circleId, user_id: userId}).done(function(data) {
//            //alert("Data Loaded: " + data);
//        });
        $.ajax({
            type: "POST",
            url: baseurl+'android/api/v3.0/'+action,
            dataType: "html",
            data: {email: auth_email_address, password: auth_password, circle_id: circleId, user_id: userId}
        }).done(function(html) {
            //alert("Data Loaded: " + html);
        });
...
//        $.ajax(baseurl+'android/api/v3.0/calenderRSVP', {email: auth_email_address, password: auth_password, event_id: event_id, type: type}).done(function(data) {
//            //alert("Data Loaded: " + data);
//        });
        $.ajax({
            type: "POST",
            url: baseurl+'android/api/v3.0/calenderRSVP',
            dataType: "html",
            data: {email: auth_email_address, password: auth_password, event_id: event_id, type: type}
        }).done(function(html) {
            //alert("Data Loaded: " + html);
        });
    ...
    {
        $.ajax("http://192.168.50.40/iseapi/android/api/v3.0/dump.php", { post_id: action_id },
        function(data) {
    ...
    $(document).ready(function(){
        ...
        newest_feed_id = $( ".bottom" ).first().attr("id");
            ...
            $(window).scroll(function(){
                ...
                var d = $(window).scrollTop() + $(document).height() + 250;
                ...
                if($("#loading").length == 0) {
//                    $('.wrap').append('<div id="loading">Loading ...'+d+' => '+t+'</div>');
                ...
                //<img alt="Loading" src="http://localhost/iseapi/android/api/v3.0/images/loading.gif">
                ...
                {    
                    ...
                    if(ajax_request_in_progress == 0) {
                        //alert('viewer_id '+logged_in_user_id+' baseurl'+ baseurl);
//                        $.ajax(baseurl+'android/api/v3.0/previousfeed',{ maxid: last_feed_id, viewer_id: logged_in_user_id, user_id: profile_user_id, event_id: event_id, circle_id: circle_id }, function(data) {
//                            alert(JSON.stringify($.base64.decode(data)));
//                            $('#loading').remove();
//                            $('.wrap').append(data);
//                        });
                        ...
                        $.ajax({
                            ...
                            dataType: "html",
                            data: {maxid: last_feed_id, viewer_id: logged_in_user_id, user_id: profile_user_id}
                        }).done(function(html) {
                            $('#loading').remove();
                            if(html != '')
                            {
