...
/*
	...
	License:
		MIT-style license.
*/
Date.prototype.myIncrement = function() {
  this.increment();
...
var Calendar = new Class({
	...
	loadCalEvents: function(){
		...
		var time = '';
		$$('div.tip').each(function(divs){if(divs.getParent().hasClass('tip-wrap')) divs.getParent().destroy();}); // Tips cleanup - tip divs kept building up after switching calendar views
		for(var i = 0; i < this.options.cEvents.length; i++){
			
			var eStart = new Date().clearTime();
			var eEnd = new Date().clearTime();
			eStart.parse(this.options.cEvents[i].start.replace(/([AM|PM]) .*/, "$1"));
			eEnd.parse(this.options.cEvents[i].end.replace(/([AM|PM]) .*/, "$1"));
...
//			console.log(eStart+"Loading events: "+this.options.cEvents[i].start +" end: "+this.options.cEvents[i].end);
...
//                                console.log("Start: " + eStart+" End: " + eEnd);
				while((eEnd.get('date') >= eStart.get('date') && eEnd.get('month') >= eStart.get('month') && eEnd.get('year') >= eStart.get('year'))
                || eEnd.get('month') > eStart.get('month') || eEnd.get('year') > eStart.get('year')){
//					console.log();
					if (eStart >= this.viewStartDate && eStart <= this.viewEndDate) {
						var eventDiv = new Element('div', {
							'html': '<a href="'+this.options.cEvents[i].href+'">'+time + ' ' + this.options.cEvents[i].title +'</a>'
						...
						if (this.options.view != 'month') // the month view only shows a portion of the event description
							...
							new Tips(eventDiv, {className: 'tip-wrap'});
							eventDiv.store('tip:title',this.options.cEvents[i].title + '<br /><i>' )
							.store('tip:text',this.options.cEvents[i].description + '<br/><br/></i>'+
									"Host: "+this.options.cEvents[i].host + '<br/></i>'+"Where: "
									+this.options.cEvents[i].location + '<br/></i>'+"Start time: "+this.options.cEvents[i].start + '<br/></i>'+
									"End time: "+this.options.cEvents[i].end + '<br/></i>'+"Category: "+this.options.cEvents[i].category + '<br/></i>'
									+this.options.cEvents[i].attending +"  Attending"+ '</i>'
									+", "+this.options.cEvents[i].maybe +"  Maybe"+ '</i>'
									+", "+this.options.cEvents[i].notattend  +" Not Attending "+ '</i>'
									+", "+this.options.cEvents[i].waiting +" Awaiting Reply"+ '</i>');
						}
					}
          // Fix mootool date library issue,
          // when increasing date to go over the last day of a month, the month does not increase
					eStart.myIncrement();
				}
