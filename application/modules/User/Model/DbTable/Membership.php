 ...
 * @license    http://www.socialengine.com/license/
 */
class User_Model_DbTable_Membership extends SharedResources_Model_DbTable_MembershipAbstract
{
...
//    }
  ...
  public function removeMember(Core_Model_Item_Abstract $resource, User_Model_User $user)
  {
    ...
    if(method_exists($api, 'deleteRoster')) {
      $api->deleteRoster($resource->getIdentity(), $user->getIdentity() . '@' . $api->getXMPPDomain());
      $api->deleteRoster($user->getIdentity(), $resource->getIdentity() . '@' . $api->getXMPPDomain());
    }
  ...
  public function setResourceApproved(Core_Model_Item_Abstract $resource, User_Model_User $user)
  {
    ...
    /* @var $api Mgslapi_Api_Openfire */
    $api = Engine_Api::_()->getApi('openfire', 'mgslapi');
    if(method_exists($api, 'addRoster')) {
      $api->addRoster($resource->getIdentity(), $user->getIdentity() . '@' . $api->getXMPPDomain());
      $api->addRoster($user->getIdentity(), $resource->getIdentity() . '@' . $api->getXMPPDomain());
    }
