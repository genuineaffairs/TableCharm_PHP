...
/**
	...
	- navigators ( forward/back ) no longer added by script. Instead, add them into overallContainer making their display from CSS and after add the CSS selector class to navs parameter
		IE: navs:{ 
				fwd:'.Sitepagealbum_SlideItMoo_forward',
				bk:'.Sitepagealbum_SlideItMoo_back'
			}
...
**/
var SitepagealbumSlideItMoo = new Class({
  ...
  options: {
    ...
    itemsSelector: null, /* css class for inner elements ( ie: .Sitepagealbum_SlideItMoo_element ) */
    ...
    navs:{ /* starting this version, you'll need to put your back/forward navigators in your HTML */
      fwd:'.Sitepagecontent_SlideItMoo_forward', /* forward button CSS selector */
      bk:'.Sitepagecontent_SlideItMoo_back' /* back button CSS selector */
    },
  ...
  setContainersSize: function(){
    ...
    if( this.options.slideVertical ){
      ...
      if( this.options.showControls ){
        var s1 = $('Sitepagealbum_SlideItMoo_forward').getSize();
        var s2 = $('Sitepagealbum_SlideItMoo_back').getSize();
    ...
    if (fwdbck_click == 1 ) {
      ...
      if (call_count == 1) {
        ...
        if(direction ==1){
          ...
          document.getElementById('Sitepagealbum_SlideItMoo_forward_disable').style.display= 'none';
          document.getElementById('Sitepagealbum_SlideItMoo_forward_loding').style.display= 'block';
        }else{
          document.getElementById('Sitepagealbum_SlideItMoo_back').style.display= 'none';
          document.getElementById('Sitepagealbum_SlideItMoo_back_disable').style.display= 'none';
          document.getElementById('Sitepagealbum_SlideItMoo_back_loding').style.display= 'block';

        } 
        var obj = this;
        var request = new Request.HTML({
          url : urlcarosel_sitepagealbum,
          method: 'GET',
          data : {
            format : 'html',
            'task': 'ajax',
          ...
          {						
            $('Sitepagealbum_SlideItMoo_items').innerHTML = responseHTML;												
            custom_sliditmoo (obj, direction, startindex_temp, forward_link);
            document.getElementById('Sitepagealbum_SlideItMoo_forward_loding').style.display= 'none';
            document.getElementById('Sitepagealbum_SlideItMoo_back_loding').style.display= 'none';
