 ...
 */
class Resume_Plugin_Job_Encode extends Core_Plugin_Job_Abstract {
  ...
  protected function _execute() {
    ...
    // Get video object
    $video = Engine_Api::_()->getItem('resume_video', $video_id);
    if (!$video || !($video instanceof Resume_Model_Video)) {
  ...
  protected function _process($video) {
    ...
    if (!$ffmpeg_path) {
      ...
      throw new Video_Model_Exception($error_msg1);
    }
    ...
    if (!@file_exists($ffmpeg_path) || !@is_executable($ffmpeg_path)) {
      ...
      if ($return > 0) {
        ...
        throw new Video_Model_Exception($error_msg2);
      }
    }
    ...
    if (!function_exists('shell_exec')) {
      ...
      throw new Video_Model_Exception($error_msg3);
    }
    ...
    if (!($video instanceof Resume_Model_Video)) {
      $error_msg6 = Zend_Registry::get('Zend_Translate')->_('Argument was not a valid video');
      throw new Video_Model_Exception($error_msg6);
    }
    ...
    if (!$success) {
      ...
      try {
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')
                ->addNotification($owner, $owner, $video, 'resume_video_processed_failed', array(
                    'message' => $notificationMessage,
                    'message_link' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'view'), 'resume_video_general', true),
                ));
    ...
    // Success
      ...
      try {
        // new action
        $subject = Engine_Api::_()->getItem('resume', $video->resume_id);
        $action = $actionsTable->addActivity($owner, $subject, 'resume_video_new');
        if ($action) {
        ...
        // notify the owner
        Engine_Api::_()->getDbtable('notifications', 'activity')
                ->addNotification($owner, $owner, $video, 'resume_video_processed');
