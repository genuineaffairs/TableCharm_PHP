  ...
  public function sendRaw(Zend_Mail $mail)
  {
    if( $this->_enabled ) {
      try {
        ...
        if(Engine_Api::_()->hasModuleBootstrap('zulu')) {
          $tmp_recipients = $mail->getRecipients();
          $mail->clearRecipients();
          
          foreach($tmp_recipients as $email) {
            // Get parent account of the email
            $parent = Engine_Api::_()->zulu()->getParentAccountByEmail($email);
            
            if($parent->getIdentity() && $parent->email) {
              $mail->addTo($parent->email);
            } else {
              $mail->addTo($email);
            }
          }
        }
//        $this->getLog()->log(print_r($mail->getRecipients(), true), Zend_Log::DEBUG);
  ...
  public function sendSystem($recipient, $type, array $params = array())
  {
    ...
    // Queue
      ...
      foreach( $recipient as $oneRecipient ) {
        if(is_string($oneRecipient)) {
          $parent = Engine_Api::_()->zulu()->getParentAccountByEmail($oneRecipient);
          if ($parent->getIdentity() && $parent->email) {
            $oneRecipient = $parent->email;
          }
        }
