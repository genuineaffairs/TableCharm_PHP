 ...
 */
class Communityad_Widget_SponserdRightAdController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.sponserd-right-ad');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_sponcered_right = $communityad_sponcered_right = Zend_Registry::get( 'communityad_sponcered_right' );
			if( empty($communityad_sponcered_right) ) {
				return $this->setNoRender();
			}
