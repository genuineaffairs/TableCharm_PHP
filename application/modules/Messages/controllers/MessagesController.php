 ...
 * @version    $Id: MessagesController.php 9902 2013-02-14 02:35:41Z shaun $
 * @author     John
 */
...
class Messages_MessagesController extends Core_Controller_Action_User
{
  ...
  public function init()
  {
    ...
    $this->view->headLink()->appendStylesheet($this->view->layout()->staticBaseUrl . 'application/modules/Messages/externals/styles/messages.css');
  }
...
//        ->getInboxPaginator($viewer);
    $this->view->paginator = $paginator = $this->_getAllMessagesPaginator();
    $paginator->setCurrentPageNumber($this->_getParam('page'));
    $this->view->unread = Engine_Api::_()->messages()->getUnreadMessageCount($viewer);

    $this->view->form = new Messages_Form_Search();
...
//      }
  ...
  public function searchAction()
  {
    ...
    $this->view->form = $form = new Messages_Form_Search();
  ...
  protected function _getAllMessagesPaginator() {
    $viewer = Engine_Api::_()->user()->getViewer();
...
//            ->where('engine4_messages_recipients.outbox_deleted != 1')
    ...
    if ($source_guid) {
      // Implicit type of Private Inbox
      if ($source_guid == 1) {
        $query->where('engine4_messages_conversations.source_type IS NULL');
      } else {
