 ...
 * @license    http://www.socialengine.com/license/
 */
class User_Model_User extends SharedResources_Model_Item_Abstract
{
  protected $_searchTriggers = array('search', 'displayname', 'username');

  protected $_localTable;
  
  protected $_isBrandedSite;
  
  public function toString($attribs = array())
  {
    if ($this->isBelongToCurrentSite()) {
      return parent::toString($attribs);
    } else {
      return "<a href='javascript:void(0)'>" . $this->getTitle() . "</a>";
    }
  }

  /**
   * Check whether this user belongs to current site
   * 
   * @return boolean
   */
  public function isBelongToCurrentSite()
  {
    $current_site_id = Engine_Api::_()->getApi('core', 'sharedResources')->getSiteId();
    return in_array($current_site_id, $this->getSiteIds());
  }

  /**
   * Get all sites user has registered
   */
  public function getSites()
  {
    return Engine_Api::_()->getDbTable('usersSites', 'sharedResources')->fetchAll(array('user_id = ?' => $this->user_id));
  }

  /**
   * Get ids of all sites this user belongs to
   * 
   * @return array
   */
  ...
  public function getMasterSiteId()
  {
    return Engine_Api::_()->getDbTable('usersSites')
                    ->fetchRow(array(
  ...
  {
    return Engine_Api::_()->getApi('core', 'sharedResources')->getSiteMaster()->getIdentity() == $this->user_id;
  }
  ...
  public function __construct($config)
  {
    ...
    $this->_localTable = clone $this->_getTable();
    $this->_localTable->setOptions(array('db' => Engine_Db_Table::getDefaultAdapter()));
    $this->_isBrandedSite = Engine_Api::_()->hasModuleBootstrap('sharedresources') &&
            Engine_Api::_()->getApi('core', 'sharedResources')->getSiteId() !== SharedResources_Model_DbTable_Sites::MASTER_SITE_ID;
  }
  ...
  public function save()
  {
    ...
    // Copy the user to local table
    if ($this->_isBrandedSite) {
      $where = $this->_getWhereQuery();
      $row = $this->_localTable->fetchRow($where);
      
      if (null === $row) {
        $this->_localTable->insert($this->_data);
      } else {
        $this->_localTable->update($this->_data, $where);
      }
    }
    return $saveResult;
  }
  ...
  public function _postDelete()
  {
    ...
    if ($this->_isBrandedSite) {
      // Delete local record as well
      $this->_localTable->delete($this->_getWhereQuery());
    }
  }
