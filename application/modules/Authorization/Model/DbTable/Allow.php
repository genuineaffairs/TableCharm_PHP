  ...
  implements Authorization_Model_AdapterInterface
{
  ...
  public function isAllowed($resource, $role, $action)
  {
    ...
    if ($resource instanceof SharedResources_Model_Item_Abstract &&
            !($resource instanceof User_Model_User) && $action == 'view') {
      return Authorization_Api_Core::LEVEL_ALLOW;
    }
    
    // If friendship is disabled on the site, always allow members to view each other's profile
    if (!Engine_Api::_()->user()->isFriendshipAllowed() && $resource instanceof User_Model_User && $action == 'view') {
      return Authorization_Api_Core::LEVEL_ALLOW;
    }
