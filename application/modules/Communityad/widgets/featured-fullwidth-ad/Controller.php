 ...
 */
class Communityad_Widget_FeaturedFullwidthAdController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.featured-fullwidth-ad');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->featured_fullwidth = $communityad_featured_fullwid = Zend_Registry::get( 'communityad_featured_fullwid' );
			if( empty($communityad_featured_fullwid) ) {
				return $this->setNoRender();
			}
