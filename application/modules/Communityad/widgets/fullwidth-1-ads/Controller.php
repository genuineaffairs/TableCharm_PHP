 ...
 */
class Communityad_Widget_Fullwidth1AdsController extends Engine_Content_Widget_Abstract
{
	...
	{ $load_content = 0;
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.fullwidth-1-ads');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_fullwidth1_ad = $communityad_fullwidth1_ad = Zend_Registry::get( 'communityad_fullwidth1_ad' );
			if( empty($communityad_fullwidth1_ad) ) {
				return $this->setNoRender();
			}
