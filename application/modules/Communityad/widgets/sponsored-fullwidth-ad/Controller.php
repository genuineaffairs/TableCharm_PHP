 ...
 */
class Communityad_Widget_SponsoredFullwidthAdController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.sponsored-fullwidth-ad');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_sponcered_fullwidth = $communityad_sponcered_fullwidth = Zend_Registry::get( 'communityad_sponcered_fullwidth' );
			if( empty($communityad_sponcered_fullwidth) ) {
				return $this->setNoRender();
			}
