 ...
 */
class Communityad_Widget_FeaturedFullwidthNoneController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
		$load_content = 0;
		$page_setting = Engine_Api::_()->communityad()->getNonWidgetLimit('communityad.featured-fullwidth-none');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->featured_fullwidthnone = $communityad_featured_fullwidnone = Zend_Registry::get( 'communityad_featured_fullwidnone' );
			if( empty($communityad_featured_fullwidnone) ) {
