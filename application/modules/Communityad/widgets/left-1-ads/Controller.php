 ...
 */
class Communityad_Widget_Left1AdsController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.left-1-ads');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_left1_ad = $communityad_left1_ad = Zend_Registry::get( 'communityad_left1_ad' );
			if( empty($communityad_left1_ad) ) {
				return $this->setNoRender();
			}
