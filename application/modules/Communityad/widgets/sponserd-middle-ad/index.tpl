...
<?php if(!empty($this->ajax_enabled)) : ?>
	...
	window.addEvent('load', function() {
		var url = en4.core.baseUrl + 'widget/index/mod/communityad/name/sponserd-middle-ad';
		var request = new Request.HTML({    
			...
			onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {
				$('sponsored_middle_col1_ajax_id').innerHTML = responseHTML;				
			}
			});  
			request.send();
	});
...
<div id="sponsored_middle_col1_ajax_id">
<?php };?>
<?php if (!empty ($this->load_content)) { ?>
	...
	<?php $div_id = 0;$div_limit = 1;$ad_count_limit = count($this->communityads_array);
		...
		foreach ($this->communityads_array as $community_ad):
			...
			<?php if($div_limit % 3  == 1){ 
				echo '<div class="caab_list_row caab_list_row_middle_sponsored">'; 
			} ?>
			<?php Engine_Api::_()->communityad()->adViewCount( $community_ad['userad_id'], $community_ad['campaign_id'] ); ?>
				<div class="caab_list">
					...
					<div id="sponsored_middle_col1_first_widget_ad_cancel_<?php echo $div_id; ?>" style="display:none;" class="cmadrem">
						<div class="cmadrem_rl">
							<?php echo '<a class="" title="' . $this->translate('Cancel reporting this ad') . '" href="javascript:void(0);" onclick="adUndo('.$div_id.', \'sponsored_middle_col1_first_widget\');">'. $this->translate("Undo"). '</a>'; ?>
						</div>
						<div class="cmadrem_con">
							<?php echo $this->translate("Do you want to report this? Why didn't you like it?"); ?>
							<form>
								<?php $ads_id = $encode_adId; ?>
								<div><input type="radio" name="adAction" value="0" onclick="adSave('Misleading', '<?php echo $ads_id; ?>', <?php echo $div_id; ?>, 'sponsored_middle_col1_first_widget')"/><?php echo $this->translate('Misleading'); ?></div>
								<div><input type="radio" name="adAction" value="1" onclick="adSave('Offensive', '<?php echo $ads_id; ?>', <?php echo $div_id; ?>, 'sponsored_middle_col1_first_widget')"/><?php echo $this->translate('Offensive'); ?></div>
								<div><input type="radio" name="adAction" value="2" onclick="adSave('Inappropriate', '<?php echo $ads_id; ?>', <?php echo $div_id; ?>, 'sponsored_middle_col1_first_widget')"/><?php echo $this->translate('Inappropriate'); ?></div>
								<div><input type="radio" name="adAction" value="3" onclick="adSave('Licensed Material', '<?php echo $ads_id; ?>', <?php echo $div_id; ?>, 'sponsored_middle_col1_first_widget')"/><?php echo $this->translate('Licensed Material'); ?></div>
								<div><input type="radio" name="adAction" value="4" onclick="otherAdCannel(4, <?php echo $div_id; ?>, 'sponsored_middle_col1_first_widget')" id="sponsored_middle_col1_first_widget_other_<?php echo $div_id; ?>"/><?php echo $this->translate('Other'); ?></div>
								<div><textarea name="sponsored_middle_col1_first_widget_other_text_<?php echo $div_id; ?>" onclick="this.value=''" onblur="if(this.value=='')this.value='<?php echo $this->translate('Specify your reason here..') ?>';"  id= "sponsored_middle_col1_first_widget_other_text_<?php echo $div_id; ?>" style="display:none;" ><?php echo $this->translate('Specify your reason here..') ?></textarea>
								</div>
								<div><?php echo '<a href="javascript:void(0);" onclick="adSave(\'Other\', \''.$ads_id.'\', '.$div_id.', \'sponsored_middle_col1_first_widget\')" id="sponsored_middle_col1_first_widget_other_button_'.$div_id.'"  style="display:none" class="cmadrem_button">' . $this->translate('Report') . '</a>'; ?></div>
							</form>
						</div>	
					</div>
			
					<!-- DIV:  Which default show, This div contain the all information about advertisment. start -->
					<div class="cmaddis" id="sponsored_middle_col1_first_widget_ad_<?php echo $div_id; ?>">
						<div class="cmaddis_close">
							<?php $is_identity = $this->viewer()->getIdentity(); 
										...
										if(!empty($is_identity)  && !empty($adcancel_enable)) { 
											echo	'<a class="" title="' . $this->translate('Report this ad') . '" href="javascript:void(0);" onclick="adCancel('.$div_id.', \'sponsored_middle_col1_first_widget\');"></a>';
										} ?>
						</div>
						...
						if( !empty($this->user_id) && !empty($community_ad['like']) && !empty($community_ad['resource_type']) && !empty($community_ad['resource_id']) && !empty($is_module_enabled) && !empty($resource_url) ) {
							...
							<?php if(empty($like_id)) { // Condition: Show 'Like Link' only when loggden user not like this advertisment. else show message that you have liked this advertismant.   ?>
								<div class="cmad_like_button" id= "<?php echo $resourceType ?>_sponsored_middle_col1_unlikes_<?php echo $encode_adId;?>" style ='display:none;' >
									<?php echo '<div class="cmaddis_cont">' .$this->translate('You like this.'). '</div>'; ?>
									<a href = "javascript:void(0);" onclick = "communityad_likeinfo('<?php echo $encode_adId; ?>', '<?php echo $resourceType ?>', '<?php echo $resource_id ?>', '<?php echo $owner_id ?>', 'sponsored_middle_col1', '<?php echo $resource_url['like']; ?>' )">
									<i class="like_thumbdown_icon"></i>
									<span><?php echo $this->translate('Unlike') ?></span>
									</a>									
								</div>
								<div class="cmad_like_button" id= "<?php echo $resourceType ?>_sponsored_middle_col1_most_likes_<?php echo $encode_adId;?>" style ='<?php echo $like_show;?>'>
									<a href = "javascript:void(0);" onclick = "communityad_likeinfo('<?php echo $encode_adId; ?>', '<?php echo $resourceType; ?>', '<?php echo $resource_id ?>', '<?php echo $owner_id ?>', 'sponsored_middle_col1', '<?php echo $resource_url['like']; ?>' )">
									<i class="like_thumbup_icon"></i>
									<span><?php echo $this->translate('Like') ?></span>
									</a>
								</div>
								<input type ="hidden" id = "sponsored_middle_col1_likeid_info_<?php echo $encode_adId;?>" value = "<?php echo $like_id; ?>"  />
							<?php }
							} 
						?>
