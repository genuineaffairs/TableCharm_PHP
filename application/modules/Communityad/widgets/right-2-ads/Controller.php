 ...
 */
class Communityad_Widget_Right2AdsController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.right-2-ads');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_right2_ads = $communityad_right2_ads = Zend_Registry::get( 'communityad_right2_ads' );
			if( empty($communityad_right2_ads) ) {
				return $this->setNoRender();
			}
