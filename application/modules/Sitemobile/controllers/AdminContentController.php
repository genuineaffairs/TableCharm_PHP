...
//      } else if ($pageRow->fragment) {
  ...
  public function updateAction() {
    ...
    try {
      ...
      $pageObject = $this->_pageTable->fetchRow($this->_pageTable->select()->where('name = ?', $page)->orWhere('page_id = ?', $page));
      if (null === $pageObject) {
      ...
      foreach ($strucure as $element) {
        ...
        // New row
          ...
          if (isset($element['module']) && $element['module'])
            $contentRow->module = $element['module'];
      ...
      foreach ($newRowsByTmpId as $tmp_id => $newRow) {
        $newData[$tmp_id] = $this->_pageTable->createElementParams($newRow);
      }
