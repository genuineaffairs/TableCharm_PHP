 ...
 */
class Communityad_Widget_FeaturedLeftAdController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.featured-left-ad');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->featured_left = $communityad_featured_left = Zend_Registry::get( 'communityad_featured_left' );
			if( empty($communityad_featured_left) ) {
				return $this->setNoRender();
			}
