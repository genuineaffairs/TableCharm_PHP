...
class User_SettingsController extends Core_Controller_Action_User
{
  ...
  public function notificationsAction()
  {
    ...
    foreach( $notificationTypes as $type ) {
      ...
      else if ($type->module == 'advgroup') {
        $elementName = 'advgroup';
        $category = 'Circles';
      }
      else if ($type->module == 'ynvideo') {
        $elementName = 'ynvideo';
        $category = 'Videos';
    ...
    $form->setAttrib('id', 'notifications_settings');

    foreach( $notificationTypesAssoc as $elementName => $info ) {
      if($info['category'] == "Events") {
        $info['category'] = "Calendar";
      }
      $form->addElement('MultiCheckbox', $elementName, array(
        'label' => $this->view->translate($info['category']),
