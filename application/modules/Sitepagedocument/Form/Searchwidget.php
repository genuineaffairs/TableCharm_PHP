 ...
 */
class Sitepagedocument_Form_Searchwidget extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setAction($view->url(array(), 'sitepagedocument_browse', true))->getDecorator('HtmlTag')->setOption('class', '');
  }
  ...
  public function getAdditionalOptionsElement() {
    ...
    $select_content = $coreContent_table->select()->where('name = ?', 'sitepagedocument.search-sitepagedocument');
    ...
    //$row = $this->_searchForm->getFieldsOptions('sitepage', 'search');
     ...
     if (in_array("4", $enabledColumns)) {
      $this->addElement('Text', 'search_document', array(
     ...
     if (in_array("3", $enabledColumns)) {
      ...
      $show_multiOptions["creation_date"] = "Everyone’s Documents";

			if(!empty($viewer_id)) {
				$show_multiOptions["my_document"] = 'My Documents';
				$show_multiOptions["my_like"] = 'Documents of Pages I Like';
			}

      $sitepagePackageEnable = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.package.enable', 1);
      if($sitepagePackageEnable) {
				$show_multiOptions["sponsored document"] = 'Sponsored Documents';
      }
      $show_multiOptions["featured"] = "Featured Documents";
      ...
      $this->addElement('Select', 'show', array(
              ...
              'onchange' => $this->gethasMobileMode() ? '' : 'searchSitepagedocuments();',
              'order' => $row->order - 2,
      ));
   ...
   if (in_array("2", $enabledColumns)) {
    ...
    if (in_array("6", $enabledColumns)) {
      $multiOptions = array();
      $multiOptions[] = '';
      foreach( Engine_Api::_()->getDbtable('categories', 'sitepagedocument')->select()->order('title ASC')->query()->fetchAll() as $row ) {    
      $multiOptions[$row['category_id']] = $row['title'];
    }
      
      $this->addElement('Select', 'document_category_id', array(
