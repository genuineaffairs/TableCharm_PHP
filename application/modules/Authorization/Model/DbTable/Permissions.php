  ...
  implements Authorization_Model_AdapterInterface
{
  ...
  protected function _getAllowed($resource, $role, $action)
  {
    ...
    // Limit the power of moderators to each site
    if ($return->value === Authorization_Api_Core::LEVEL_MODERATE && $resource instanceof Core_Model_Item_Abstract && $role instanceof User_Model_User &&
            Engine_Api::_()->getDbtable('levels', 'authorization')->fetchRow(array('level_id = ?' => $level_id))->type === 'moderator' &&
            !$role->isSiteMaster()) {
      $return->value = Authorization_Api_Core::LEVEL_ALLOW;
    }
