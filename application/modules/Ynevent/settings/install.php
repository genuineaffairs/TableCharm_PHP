...
class Ynevent_Installer extends Engine_Package_Installer_Module
{
    ...
    public function onInstall()
    {
        ...
        $page_id = $db->select()
                ...
                ->where('name = ?', 'ynevent_index_browse')
                ->limit(1)
                ->query()
                ->fetchColumn();
		if ($page_id) {
			$featured_widget_id = $db->select()
						                ->from('engine4_core_content', 'content_id')
						                ->where('page_id = ?', $page_id)
						                ->where('type = ?', 'widget')
						                ->where('name = ?', 'ynevent.feature-events')
						                ->limit(1)
						                ->query()
						                ->fetchColumn();
	...
	protected function _addEventCreatePage()
  {
    ...
    $page_id = $db->select()
      ->from('engine4_core_pages', 'page_id')
      ->where('name = ?', 'ynevent_index_create')
      ->limit(1)
      ->query()
      ->fetchColumn();
	...
	protected function _addEventManagePage()
  {
    ...
    $page_id = $db->select()
      ->from('engine4_core_pages', 'page_id')
      ->where('name = ?', 'ynevent_index_manage')
      ->limit(1)
      ->query()
      ->fetchColumn();
