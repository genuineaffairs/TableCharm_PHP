...
(function( $, window, undefined ) {
  ...
  TR.applyColor = function( color, prefix ) {
    ...
    if( element != "link" ) {
      ...
      $( ".slider[data-name=" + prefix + "-background-color]" ).slider( {
        value: 50
      } )
      .find( "a" ).css({
        "background": color, 
  ...
  TR.correctNumberOfSwatches = function() {
    ...
    var matches = style.match( /\/\*\s[A-E]\s*-*\*\//g );
    if( !matches ) {
      if(TR.firstLoad) {
        TR.firstLoad = 0;
  ...
  TR.initAddSwatch = function() {
    ...
    TR.iframe.find( "#add-swatch" ).hide();
  }
  ...
  TR.initDialogs = function() {
    ...
    $( "#share-button" ).click(function(e) {
      ...
      var post_data = "body=" + TR.styleBlock.text();
      $.ajax({
        type: "post",
        url: en4.core.baseUrl+"admin/sitemobile/themes/roller-save",
        data: post_data,
        beforeSend: function() {
  ...
  TR.initThemeRoller = function() {
    ...
    //with the inspector
    var starting_swatches = ["a", "b", "c", "d", "e"];
    for( var i = 0; i < 5; i++ ) {
  ...
  TR.updateThemeRoller = function( tab ) {
    ...
    $( ".delete-swatch-" + TR.alpha[tab - 1] ).hide();
    $( ".duplicate-swatch-" + TR.alpha[tab - 1] ).hide();
