 ...
 */
class Communityad_Widget_Middle1AdsController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.middle-1-ads');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_middle1_ad = $communityad_middle1_ad = Zend_Registry::get( 'communityad_middle1_ad' );
			if( empty($communityad_middle1_ad) ) {
				return $this->setNoRender();
			}
