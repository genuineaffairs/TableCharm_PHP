...
//        $db ->query("UPDATE `engine4_core_modules` SET `enabled`= 0 WHERE `engine4_core_modules`.`name` = 'video';");
    ...
    protected function _checkFfmpegPath() {
        ...
        if (function_exists('exec') && function_exists('shell_exec')) {
            ...
            $ffmpeg_path = $db->select()
                    ...
                    ->where('name = ?', 'ynvideo.ffmpeg.path')
                    ->limit(1)
                    ->query()
                    ->fetchColumn(0)
            ;
    ...
    protected function _addUserProfileContent() {
        ...
        $select->from('engine4_core_content')
                ->where('page_id = ?', $page_id)
                ->where('type = ?', 'widget')
                ->where('name = ?', 'ynvideo.profile-videos');
        ...
        $select->reset('where')
                ->where('page_id = ?', $page_id)
                ->where('type = ?', 'widget')
                ->where('name = ?', 'ynvideo.profile-favorite-videos');
        ...
        $select->reset('where')
                ->where('page_id = ?', $page_id)
                ->where('type = ?', 'widget')
                ->where('name = ?', 'ynvideo.profile-video-playlists');
        $infoProfileFavoriteVideos = $select->query()->fetch();
        if (empty($infoProfileFavoriteVideos)) {
...
//        $db->query("UPDATE `engine4_core_modules` SET `enabled`= 0 WHERE `engine4_core_modules`.`name` = 'video';");
    ...
    private function _addMenuItems() {
        ...
        $select1->from('engine4_core_menuitems')
                ->where('name = ?', 'core_main_ynvideo')
                ->limit(1);
        $info = $select1->query()->fetch();
        if (empty($info)) {
            $db->insert('engine4_core_menuitems', array(
                'name' => 'core_main_ynvideo',
                'module' => 'ynvideo',
                'label' => 'Videos',
                'params' => '{"route":"video_general"}',
                'menu' => 'core_main'
            ));
        }
        
        $select2 = new Zend_Db_Select($db);        
        $select2->from('engine4_core_menuitems')
                ->where('name = ?', 'core_admin_main_plugins_ynvideo')
                ->limit(1);
        $info = $select2->query()->fetch();
        if (empty($info)) {
            $db->insert('engine4_core_menuitems', array(
                'name' => 'core_admin_main_plugins_ynvideo',
                'module' => 'ynvideo',
                'label' => 'Videos',
                'params' => '{"route":"admin_default","module":"ynvideo","controller":"manage"}',
    ...
    private function _addNonCategory() {
        ...
        $select = new Zend_Db_Select($db);        
        $select->from('engine4_video_categories')
                ->where('category_id = 0')
                ->limit(1);
    ...
    protected function _synchronizeStatisticData() {
        ...
        $db->query("UPDATE `engine4_video_videos` AS `videos` "
                . "SET `favorite_count` = "
                . "(SELECT COUNT(*) from `engine4_video_favorites` AS `favorites` WHERE `favorites`.video_id = `videos`.video_id)");
        
        // update video_count for playlists
        $db->query("UPDATE `engine4_video_playlists` AS `playlists` "
                . "SET `video_count` = "
                . "(SELECT COUNT(*) from `engine4_video_playlistassoc` AS `playlistassoc` WHERE `playlists`.playlist_id = `playlistassoc`.playlist_id)");
        ...
        foreach($select->query()->fetchAll() as $userId) {
            ...
            $selectCountVideo->from('engine4_video_videos', 'count(*)')
                    ->where('owner_id = ?', $userId['owner_id']);
            $query = $selectCountVideo->query();
            $videoCount = $query->fetchColumn();
            $db->insert('engine4_video_signatures', array(
    ...
    protected function _addVideoIndexPage() {
        ...
        // Check if it's already been placed
        $select->from('engine4_core_pages')
                ->where('name = ?', 'ynvideo_index_index')
                ->limit(1);
        $info = $select->query()->fetch();

        if (empty($info)) {
            $db->insert('engine4_core_pages', array(
                'name' => 'ynvideo_index_index',
                'displayname' => 'Advanced Video Home Page',
                'title' => 'Advanced Video',
                'description' => 'This is the home page for the advanced video.',
                'custom' => 0,
            ));
            ...
            $db->insert('engine4_core_content', array(
                ...
                'order' => 1,
                'params' => '{"title":"","slidingDuration":"5000","nomobile":"0","name":"ynvideo.list-featured-videos","slideWidth":"740","slideHeight":"400"}',
            ));
            ...
            $db->insert('engine4_core_content', array(
                ...
                'order' => 1,
                'params' => '{"title":"Latest Videos","recentType":"creation","nomobile":"0","itemCountPerPage":"12","name":"ynvideo.list-recent-videos"}',
            ));
            ...
            $db->insert('engine4_core_content', array(
                ...
                'parent_content_id' => $container_tab_id,
                'order' => 2,
                'params' => '{"title":"Most Viewed","popularType":"view","nomobile":"0","itemCountPerPage":"4","name":"ynvideo.list-popular-videos","viewType":"big"}',
            ));
            ...
            $db->insert('engine4_core_content', array(
                ...
                'parent_content_id' => $container_tab_id,
                'order' => 3,
                'params' => '{"title":"Most Liked","numberOfVideos":"5","nomobile":"0","name":"ynvideo.list-liked-videos","viewType":"big"}',
            ));
            ...
            $db->insert('engine4_core_content', array(
                ...
                'parent_content_id' => $container_tab_id,
                'order' => 4,
                'params' => '{"title":"Most Rated","popularType":"rating","nomobile":"0","itemCountPerPage":"4","name":"ynvideo.list-popular-videos","viewType":"big"}',
            ));
            ...
            $db->insert('engine4_core_content', array(
                ...
                'parent_content_id' => $container_tab_id,
                'order' => 5,
                'params' => '{"title":"Most Commented","popularType":"comment","nomobile":"0","itemCountPerPage":"4","name":"ynvideo.list-popular-videos","viewType":"big"}',
            ));
            ...
            $db->insert('engine4_core_content', array(
                ...
                'parent_content_id' => $right_id,
                'order' => 3,
                'params' => '{"title":"Top Members","name":"ynvideo.list-top-members"}',
            ));
        }
    }
	...
	protected function _addVideoCreatePage()
	{
    	...
    	$page_id = $db->select()
      		...
      		->where('name = ?', 'ynvideo_index_create')
		    ->limit(1)
		    ->query()
		    ->fetchColumn();
	    ...
	    if( !$page_id ) {
		      // Insert page
		      $db->insert('engine4_core_pages', array(
		        'name' => 'ynvideo_index_create',
		        'displayname' => 'Advanced Video Create Page',
		        'title' => 'Video Create',
    ...
    protected function _addMyVideosPage() {
        ...
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_pages')
                ->where('name = ?', 'ynvideo_index_manage')
                ->limit(1);
        $info = $select->query()->fetch();

        if (empty($info)) {
            $db->insert('engine4_core_pages', array(
                'name' => 'ynvideo_index_manage',
                'displayname' => 'Advanced Video My Videos Page',
                'title' => 'My Videos',
    ...
    protected function _addVideoListingPage() {
        ...
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_pages')
                ->where('name = ?', 'ynvideo_index_list')
                ->limit(1);
        $info = $select->query()->fetch();

        if (empty($info)) {
            $db->insert('engine4_core_pages', array(
                'name' => 'ynvideo_index_list',
                'displayname' => 'Advanced Video Listing Page',
                'title' => 'Listing Videos',
                'description' => 'This is the advanced video listing page.',
                'custom' => 0,
            ));
            $page_id = $db->lastInsertId('engine4_core_pages');
    ...
    protected function _addVideoFavoritePage() {
        ...
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_pages')
                ->where('name = ?', 'ynvideo_favorite_index')
                ->limit(1);
        $info = $select->query()->fetch();

        if (empty($info)) {
            $db->insert('engine4_core_pages', array(
                'name' => 'ynvideo_favorite_index',
                'displayname' => 'Advanced Video Favorite Page',
    ...
    protected function _addVideoPlaylistsPage() {
        ...
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_pages')
                ->where('name = ?', 'ynvideo_playlist_index')
                ->limit(1);
        $info = $select->query()->fetch();

        if (empty($info)) {
            $db->insert('engine4_core_pages', array(
                'name' => 'ynvideo_playlist_index',
                'displayname' => 'Advanced Video Video Playlists Page',
                'title' => 'View Video Playlists',
    ...
    protected function _addVideoPlaylistDetailsPage() {
        ...
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_pages')
                ->where('name = ?', 'ynvideo_playlist_view')
                ->limit(1);
        $info = $select->query()->fetch();

        if (empty($info)) {
            $db->insert('engine4_core_pages', array(
                'name' => 'ynvideo_playlist_view',
                'displayname' => 'Advanced Video View Playlist Detail Page',
    ...
    protected function _addVideoWatchlaterPage() {
        ...
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_pages')
                ->where('name = ?', 'ynvideo_watch-later_index')
                ->limit(1);
        $info = $select->query()->fetch();

        if (empty($info)) {
            $db->insert('engine4_core_pages', array(
                'name' => 'ynvideo_watch-later_index',
                'displayname' => 'Advanced Video Watch Later Page',
    ...
    protected function _addVideoDetailPage() {
        ...
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_pages')
                ->where('name = ?', 'ynvideo_index_view')
                ->limit(1);
        $info = $select->query()->fetch();

        if (empty($info)) {
            $db->insert('engine4_core_pages', array(
                'name' => 'ynvideo_index_view',
                'displayname' => 'Advanced Video View Page',
                'title' => 'View Video',
