...
<?php
  ...
  <?php if( !$this->noList ): ?><li class="<?php echo 'Hide_'.$item->getType()."_".$item->getIdentity() ?>" id="activity-item-<?php echo $action->action_id ?>"  data-activity-feed-item="<?php echo $action->action_id ?>" <?php if($this->onViewPage): ?>style="padding-bottom: 30px;" <?php endif; ?> ><?php endif; ?>
    ...
    <div class='feed_item_photo'> <?php echo  $this->htmlLink($itemPhoto->getHref(),
      $this->itemPhoto($itemPhoto, 'thumb.icon', $itemPhoto->getTitle()),  array('class'=>'sea_add_tooltip_link', 'rel'=>$itemPhoto->getType().' '.$itemPhoto->getIdentity())
    )  ?></div>
...
?>"><i class="aaf_privacy_pulldown_icon aaf_icon_feed_custom"></i>
    ...
    <div class='feed_item_body'>
      ...
      <div class='feed_item_date feed_item_icon <?php echo $icon_type ?>'>
        <ul>
          <?php // Check if the feed is of resume type ?>
          <?php $firstAttachment = $action->getFirstAttachment() ?>
          <?php if($firstAttachment && $firstAttachment->meta->type !== 'resume') : ?>
          <?php if( $canComment ): ?>
...
array('onclick'=>'javascript:en4.advancedactivity.like(this,'.$action->action_id.');', 'title' =>
...
$this->translate('ADVACADV_SHARE'), array('class' => 'smoothbox', 'title' => $this->translate('Share this by re-posting it with your own message.'))) ?>
          ...
          <?php // if feed is not resume ?>
          <?php elseif($firstAttachment->item instanceof Resume_Model_Resume) : ?>
          <li>
            <?php echo $this->translate(array('%s view', '%s views', $firstAttachment->item->view_count), $this->locale()->toNumber($firstAttachment->item->view_count)); ?>
            <span>&#183;</span>
          </li>
          <?php endif; ?>
      ...
      <?php if( ($action->getTypeInfo()->shareable && $action->shareable &&  ($share = $sharesTable->countShareOfItem(array ('parent_action_id' => $action->getIdentity())))>0) || ($action->getTypeInfo()->commentable && $action->commentable)) : // Comments - likes -share?>
        <div id='comment-likes-activityboox-item-<?php echo $actionBaseId ?>' class='comments' <?php if(!$this->viewAllLikes && !$this->viewAllComments && Engine_Api::_()->getApi('settings', 'core')->getSetting('aaf.comment.like.box', 0)):?>style="display: none;"  <?php endif; ?>>
          <ul>  
          	<?php // Share Count ?>
