 ...
 * @license    http://www.socialengine.com/license/
 */
class Zulu_Form_Signup_Account extends Engine_Form
{
  protected $_subAccountMode = true;
  ...
  public function init()
  {
    ...
    // Add fields to support multiple accounts using one email functionality
    if($this->_subAccountMode) {
      Zulu_Form_Helper::addSubAccountFields($this);
    }
    ...
    if( count($topStructure) == 1 && $topStructure[0]->getChild()->type == 'profile_type' ) {
      ...
      // Zulu hack for excluding Profiles which do not belong to Zulu
      foreach($options as $key => $opt) {
          if(preg_match('/^(?!Zulu)/', ucfirst($opt->label))) {
              unset($options[$key]);
          }
      }
      ...
      if( count($options) > 1 ) {
        ...
        // Zulu hack for excluding Profiles which do not belong to Zulu
        foreach($options['options']['multiOptions'] as $key => $opt) {
            if(preg_match('/^(?!Zulu)/', ucfirst($opt))) {
                unset($options['options']['multiOptions'][$key]);
            }
        }
        ...
        $this->addElement('Hidden', 'profile_type', array(
          'value' => $options[0]->option_id,
            // Zulu hack to make sure that this hidden field will be displayed
            'order' => 1000,
        ));
      }
    }
...
//      }
    ...
    // Set default action
    $this->setAction(Zend_Controller_Front::getInstance()->getRouter()->assemble(array('is_children_account' => $is_child_mode), 'zulu_signup', true));
  }
  
  public function isValid($data) {
    // add/remove validators to/from sub account fields
    if($this->_subAccountMode) {
      Zulu_Form_Helper::checkSubAccountFields($this, $data);
    }
