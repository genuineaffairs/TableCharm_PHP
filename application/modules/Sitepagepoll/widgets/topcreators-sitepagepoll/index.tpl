...
<ul class="sitepage_sidebar_list">
  <?php foreach ($this->paginator as $poll):?>
        <li>
					<?php $sitepage_object = Engine_Api::_()->getItem('sitepage_page', $poll['page_id']);?>
					<?php $layout = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.layoutcreate', 0);
					$tab_id = Engine_Api::_()->sitepage()->GetTabIdinfo('sitepagepoll.profile-sitepagepolls', $poll['page_id'], $layout);?>
            ...
            <?php echo $this->htmlLink($sitepage_object->getHref(), Engine_Api::_()->sitepage()->truncation($sitepage_object->getTitle()), array('title' => $sitepage_object->getTitle())); ?> 
						...
						<div class="sitepage_sidebar_list_details">
						 <?php echo $this->htmlLink($sitepage_object->getHref(array('tab'=> $tab_id)),$poll['item_count'].' polls'); ?>
				    </div>	
      </li>
	<?php endforeach; ?>
</ul>