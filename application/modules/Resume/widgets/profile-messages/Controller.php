...
class Resume_Widget_ProfileMessagesController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $resumer_owner_link = $resumer_owner->toString(array('target' => '_blank'));
    ...
    try {
      ...
      // Get site master
      $site_master = Engine_Api::_()->getApi('core', 'sharedResources')->getSiteMaster($resume->site_id);

      // If the site where the resume was created has a site master
      if ($site_master instanceof User_Model_User && $site_master->getIdentity()) {
        // Send the message to the site master
        $recipients = $site_master;
      } else {
        // Otherwise, send it to the resume owner
