 ...
 */
class Communityad_Widget_Left2AdsController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
	  ...
	  if( empty($identity) ) {
		...
		$page_setting = Engine_Api::_()->communityad()->getWidgetLimit($this->view->identity, 'communityad.left-2-ads');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_left2_ad = $communityad_left2_ad = Zend_Registry::get( 'communityad_left2_ad' );
			if( empty($communityad_left2_ad) ) {
				return $this->setNoRender();
			}
