 ...
 * @license    http://www.socialengine.com/license/
 */
class Mgslapi_View_Helper_Customtimestamp extends Zend_View_Helper_HtmlElement
{
  const MINUTE = 60;
  const HOUR = 3600;
  const DAY = 86400;
  ...
  public function customtimestamp($time = null, $attribs = array(), $lang)
  {
    if( null === $time )
    {
      $time = time();
    ...
    $content = $this->calculateDefaultTimestamp($time, $lang);
  ...
  public function calculateDefaultTimestamp($time, $lang)
  {
    $now = time();
    ...
    else if( $delta < self::WEEK && $tsWeek == $nowWeek )
    {
      ...
      return $this->view->translate(
        '%s at %s',
        $dayOfWeek,
        $this->view->locale()->toTime($time, array('size' => 'short', 'locale' => $lang)),
        $lang
      );
    }
    ...
    {
      return $this->view->locale()->toTime($time, array('type' => 'dateitem', 'size' => 'MMMMd', 'locale' => $lang));
    }

    // Otherwise use the full date
    else
    {
      return $this->view->locale()->toDate($time, array('size' => 'long','locale' => $lang));
    }
    ...
    if( $translator ) {
      return $translator->translate($key, $val, $lang);
    } else {
