 ...
 */
class Communityad_Widget_SponsoredLeftNoneController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
		$load_content = 0;
		$page_setting = Engine_Api::_()->communityad()->getNonWidgetLimit('communityad.sponsored-left-none');
		...
		if (!empty($_GET['load_content']) || empty($ajax_enabled)) {
		  $load_content = 1;
			...
			$this->view->communityad_sponcered_leftnone = $communityad_sponcered_leftnone = Zend_Registry::get( 'communityad_sponcered_leftnone' );
			if( empty($communityad_sponcered_leftnone) ) {
