...
class Mgslapi_AndroidapiController extends Mgslapi_Controller_Action_Api {

  private $_currentModuleCoreApi;
  private $_viewer;
  protected $_deviceType = Mgslapi_Model_DbTable_Devices::ANDROID_DEVICE_TYPE;
...
//        { 
//            $image = explode('?c=', Engine_Api::_()->mgslapi()->getItemPhotoUrl($user)); 
//            $response = array();
...
//            $response['response']['data']['user_image'] = $this->mybase64_encode($image[0]);                    
...
//    }
    ...
    if ($viewer = $this->checkAuth()) {
      ...
      if ($subject) {
        $html = $this->view->partial(
                'android/_view_profile_feed.tpl', $data
        );
      } else {
        $html = $this->view->partial(
                'android/_view_feed.tpl', $this->_currentModuleCoreApi->getModuleName(), $data
        );
      }
  ...
  public function getcalenderfeedAction() {
    if ($viewer = $this->checkAuth()) {
      ...
      $data = $this->allfeed($viewer, $subject);
      $html = $this->view->partial(
              'android/_view_event_feed.tpl', $this->_currentModuleCoreApi->getModuleName(), $data
      );
  ...
  public function getlatestfeedAction() {
    if ($viewer = $this->checkAuth()) {
      ...
      if (!$data['activityCount']) {
        ...
        $html = $this->view->partial(
                'android/_view_latest_feed.tpl', $this->_currentModuleCoreApi->getModuleName(), $data
        );
  ...
  public function previousfeedAction() {
    ...
    $html = $this->view->partial(
            'android/_dumy_feed.tpl', $this->_currentModuleCoreApi->getModuleName(), array(
        'activity' => $activity,
        'activityCount' => count($activity),
        'nextid' => $nextid,
        'firstid' => $firstid,
    ...
    if ($viewer = $this->checkAuth()) {
      ...
      if ($total > 0) {
        ...
        foreach ($photos as $photo) {
          $imageNormal = explode('?c=', Engine_Api::_()->mgslapi()->getItemPhotoUrl($photo, 'thumb.normal'));
          $imageMain = explode('?c=', Engine_Api::_()->mgslapi()->getItemPhotoUrl($photo, 'thumb.main'));
          $response['response']['data'][$i]['photo_id'] = $photo->photo_id;
          $response['response']['data'][$i]['album_id'] = $album->album_id;
          $response['response']['data'][$i]['photo_thumb_url_encoded'] = $this->mybase64_encode($imageNormal[0]);
          $response['response']['data'][$i]['photo_big_url_encoded'] = $this->mybase64_encode($imageMain[0]);
          $response['response']['data'][$i]['photo_thumb_url'] = $imageNormal[0];
          $response['response']['data'][$i]['photo_big_url'] = $imageMain[0];
...
//                $total = count($photos);
      ...
      $html = $this->view->partial(
              'android/_view_photo_comment.tpl', $this->_currentModuleCoreApi->getModuleName(), array('comments' => $comments,
          'viewer' => $viewer,
  ...
  public function postfeedlikeAction() {
    if ($viewer = $this->checkAuth()) {
      ...
      try {
        ...
        if (!$action->action_id) {
          return;
          //$this->JSONErrorOutput('Feed not found'); 
        }
        // Action
        if (!$comment_id) {
          // Check authorization
...
//              }
  ...
  public function getcirclefeedAction() {
    if ($viewer = $this->checkAuth()) {
      ...
      $data = $this->allfeed($viewer, $subject);
      $html = $this->view->partial(
              'android/_view_circle_feed.tpl', $this->_currentModuleCoreApi->getModuleName(), $data
      );
  ...
  public function getallunseenfriendrequestsAction() {
    if ($viewer = $this->checkAuth()) {
      ...
      $requests->setItemCountPerPage(50);
      $i = 0;
      $response = array();
      if ($total > 0) {
        $response['response']['success'] = 'success';
...
//              $publishMessage = html_entity_decode($form->getValue('body'));
  ...
  public function __postuserlocationAction() {
    if ($viewer = $this->checkAuth()) {
      exit('need to talk');
      // Get subject if necessary
      $strName = str_replace('www.', '', strtolower($_SERVER['HTTP_HOST']));
...
//              $publishMessage = html_entity_decode($form->getValue('body'));
      ...
      if ($action) {
        ...
        $response['response']['success'] = 'success';
        $response['response']['data']['post_fail'] = $post_fail;
        $response['response']['data']['feed_stream'] = $feed_stream;
        $response['response']['data']['last_id'] = $last_id;
        $response['response']['hash'] = md5(serialize($response));
        $this->_jsonSuccessOutput($response);
      } else {
        $this->_jsonErrorOutput('Post not possible');
...
//        $this->_jsonErrorOutput('device_token id required');
//      }
...
//          $registeredDevice->device_type = 2;
...
//        //$device = $this->view->userAgent()->getDevice();
//        //$browser = $device->getBrowser();
//        //$browsers = array('Chrome', 'Firefox');
////        if(in_array($browser, $browsers))
////        {
////            echo $this->prettyPrint( json_encode($response) );
////        }
////        else
////        {
