...
//     }
  ...
  public function tinymceEditorOptions($upload_url = false) {
    ...
    if ($coreversion < '4.7.0') {
      ...
      return array(
          'upload_url' => $upload_url,
          'menubar' => true,
          'plugins' => "advlist,autolink,lists,link,image," . $uploadPlugin . "charmap,print,preview,hr,anchor,pagebreak,searchreplace,wordcount,visualblocks,visualchars,code,fullscreen,insertdatetime,media,nonbreaking,save,table,contextmenu,directionality,emoticons,paste,textcolor",
          'toolbar1' => "undo,redo,removeformat,pastetext,|,code,link,media,image," . $uploadPlugin . "emoticons,|,bullist,numlist,|,print,preview,fullscreen",
  ...
  public function getMyLocationDetailsCookie() {
    ...
    if (Count($seaocore_myLocationDetails) > 1) {
      ...
      if (!empty($seaocore_myLocationDetails)) {
        $seaocore_myLocationDetails['notInSearch'] = isset($seaocore_myLocationDetails['notInSearch']) ? $locationArray['notInSearch'] : 0;
      }
      return $seaocore_myLocationDetails;
    }
