...
<ul class="sitepage_sidebar_list">
  <?php foreach ($this->recentlyview as $event): ?>
    <li>    
      <?php echo $this->htmlLink($event->getHref(), $this->itemPhoto($event, 'thumb.icon'), array('title' => $event->title)) ?>
  ...
  <li class="sitepage_sidebar_list_seeall">
		<a href='<?php echo $this->url(array('sponsoredevent'=> 1), 'sitepageevent_browse', true) ?>'><?php echo $this->translate('See All');?> &raquo;</a>
	</li>
</ul>