...
class Ynevent_Model_DbTable_Events extends Engine_Db_Table {
    ...
    public function getEventSelect($params = array()) {
        ...
        $select->setIntegrityCheck(false)
        	->from("$eventTableName", array("$eventTableName.*"))
        	->join("$userTableName", "$userTableName.user_id = $eventTableName.user_id", '');
    ...
    public function getMyEventsInMonth($user_id, $from, $to) {
        ...
        $select->setIntegrityCheck(false)->from($eventTableName, array(
            "$eventTableName.*", "date($eventTableName.starttime) as event_day", "date_format($eventTableName.starttime,'%h:%i %p') as event_time"
    ...
    public function getRecurrenceInMonth($user_id, $event, $from, $to){
        ...
        $select = $this->select();
        $select -> setIntegrityCheck(false) 
        		-> from($eventTableName, array("$eventTableName.*", "date($eventTableName.starttime) as event_day", "date_format($eventTableName.starttime,'%h:%i %p') as event_time"
        		))
        		-> where("$eventTableName.starttime between '$from' and '$to'")
    ...
    public function getEvents($user_id, $from, $to) {
        ...
        $select->setIntegrityCheck(false)->from($eventTableName, array(
                ...
                ))->join($membershipTableName, "$eventTableName.event_id = $membershipTableName.resource_id", '')->where("$membershipTableName.user_id = ?", $user_id)
                ->where("$membershipTableName.active = 1 and $membershipTableName.rsvp <> 0")->order("$eventTableName.starttime");
	...
	public function getGeneralCalendar($fromdate, $todate)
	{
		...
		$select->setIntegrityCheck(false)
			->from("$eventTableName", array("$eventTableName.event_id", "$eventTableName.starttime", "$eventTableName.endtime" ))
			->join("$userTableName", "$userTableName.user_id = $eventTableName.user_id", array());
		$select->where("$eventTableName.starttime between '$fromdate' and '$todate' ");
